...
그 이후 클라이언트가 그 Socket의 연결을 오픈하고, 클리퍼에서 명령을 보낼 수 있습니다. 
...
만일 "id" 가 존재한다면 클리퍼는 "id"를 포함한 응답 메시지와 함께 요청에 응답할 것입니다.
...
만일 요청한 프로세싱이 오류면,
응답메시지는 "result" 필드 대신에 "error" 필드를 포함할 것입니다. 
예를 들어, 다음과 같은 요청이 들어가면 : 
...
클리퍼는 항상 받은 순서대로 요청 프로세싱을 시작합니다. 
하지만 몇몇 요청들은 즉각 완료되지 않을 수 있습니다. 
이로인해 관련응답이 다른 요청에 대한 응답순서를 벗어날 수도 있습니다. 
JSON 요청은 절대로 미래의 JSON 요청의 프로세싱을 중단시키지 않을 것입니다. 
...
### 정보 (info)
 ...
 "info" endpoint 는 클리퍼로 부터 시스템과 버전 정보를 가져올 수 있습니다. 
또한 클리퍼에 클라이언트 버전 정보르 제공하는데도 사용될 수 있습니다. 
예시 :
`{"id": 123, "method": "info", "params": { "client_info": { "version":
"v1"}}}`

만일 정보가 존재한다면, "client_info" 파라메터는 딕셔너리여야만 합니다. 
